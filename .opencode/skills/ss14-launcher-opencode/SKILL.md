---
name: ss14-launcher-opencode
description: Expert coding guidance for the Space Station 14 launcher repository. Use when OpenCode edits, reviews, debugs, tests, refactors, or plans code in SS14.Launcher, SS14.Loader, SS14.Launcher.Bootstrap, Robust.LoaderApi, build scripts, Avalonia XAML, localization, SQLite migrations, updater, authentication, server browser, content download, process launch, packaging, or release workflows.
license: MIT
metadata:
  compatibility: opencode
  project: SS14.Launcher
  stack: dotnet-avalonia
---

# SS14 Launcher OpenCode
...
- Load this skill for code work in this repository, then inspect local files before editing.
- Use `rg` and targeted file reads from the current working directory. OpenCode discovers project skills while walking toward the git root, so keep skill names unique across `.opencode/skills`, `.claude/skills`, and `.agents/skills`.
- If adding or changing OpenCode skills later, keep directory names equal to `name`, use lowercase hyphen-case, keep `description` under 1024 characters, and do not create duplicate names in compatibility locations.
- Do not add or loosen `opencode.json` skill permissions unless the user asks. A project-local skill should be usable without broad permission changes in most setups.
- Keep edits scoped. Do not rewrite broad areas, reformat unrelated XAML, upgrade dependencies, or change packaging unless the request requires it.
